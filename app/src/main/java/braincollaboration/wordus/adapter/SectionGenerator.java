...
class SectionGenerator {
     ...
     * @return the original list of objects with {@link SectionedAdapterBase.SectionHeader}s inserted
     */
    static List<Object> getSectionsForItems(List<? extends Categorizable> items) {
        ...
        for (int i = 0; i < items.size() + sectionHeaders.size(); i++) {
            if (sectionHeaders.containsKey(i)) {
                listItems.add(new SectionedAdapterBase.SectionHeader(sectionHeaders.get(i)));
                sectionCount++;
            } else {
